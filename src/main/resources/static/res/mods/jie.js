...
layui.define('fly', function(exports){
  ...
  //解答操作
  gather.jiedaActive = {
    reply: function(div){ //回复  	
      var val = dom.content.val();
      var aite = '@'+ div.find('#replyuser cite').text().replace(/\s/g, '');
      var username =  div.find('#replyuser').attr("href").split('/')[1];
      var commentid =  div.parents("li").attr("commentid");
	  $("#L_content").attr("touser",username);
	  $("#L_content").attr("commentid",commentid);
      console.log(aite);
      dom.content.focus()
  ...
  $('.jieda-reply #reply').on('click', function(){
	  var username=$('.fly-nav-user li').attr('username');
		if(username==null){
			layer.msg("请登录后再进行点赞操作!", { icon: 2, time: 1000 });
		}else{
			var othis = $(this), type = othis.attr('type');
			gather.jiedaActive[type].call(this, othis.parent().siblings(".detail-about"));
		}
  ...
  if(/\/page\//.test(location.href) && !location.hash){
    var replyTop = $('#flyReply').offset().top - 10;
